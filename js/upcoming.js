...
function search(data) {
  ...
  if (searchTerm === "") {
      ...
      data.forEach(function (event) {
          if (new Date(event.date) >= new Date()) { // Verificar si el evento es futuro
              var card = generateCard(event);
              cardContainer.innerHTML += card;
          }
      });
      ...
      var found = false;
      data.forEach(function (event) {
          if (event.name.toLowerCase().includes(searchTerm) && new Date(event.date) >= new Date()) { // Verificar si el evento es futuro
              var card = generateCard(event);
              cardContainer.innerHTML += card;
              found = true;
          }
